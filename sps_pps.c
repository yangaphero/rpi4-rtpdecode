...
{
	if(!sps_ptr->pic_width_in_mbs_minus1) return 0;//自定义
...
{
	if(!sps_ptr->pic_height_in_map_units_minus1) return 0;//自定义
...
int h264_get_profile_idc(SPS *sps_ptr)
{
    return (sps_ptr->profile_idc);
}

int h264_get_level_idc(SPS *sps_ptr)
{
    return ( sps_ptr->level_idc);
}
